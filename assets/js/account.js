(function () {
  ...
  function fillRegions(form, selectedRegion, selectedCommune) {
    ...
    region.innerHTML = '<option value="">Selecciona una región</option>' +
      TCG_REGIONS.map(item => `<option value="${escapeHtml(item.region)}">${escapeHtml(item.region)}</option>`).join('');
    ...
    function updateCommunes(communeToSelect) {
      ...
      commune.innerHTML = '<option value="">Selecciona una comuna</option>' +
        communes.map(name => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');
      if (communeToSelect && communes.includes(communeToSelect)) commune.value = communeToSelect;
    }
  ...
  function setupProfile() {
    ...
    if (form) {
      ...
      form.addEventListener('submit', event => {
        ...
        let valid = true;
        const checks = [
          [form.elements.run, !run ? 'RUN es requerido.' : (run.length < 7 || run.length > 9 || !TCG.validateRun(run)) ? 'RUN no válido. Escríbelo sin puntos ni guion.' : ''],
          [form.elements.firstName, !firstName ? 'Nombre es requerido.' : firstName.length > 50 ? 'Nombre permite máximo 50 caracteres.' : ''],
          [form.elements.lastName, !lastName ? 'Apellidos son requeridos.' : lastName.length > 100 ? 'Apellidos permiten máximo 100 caracteres.' : ''],
          [form.elements.email, !email ? 'Correo es requerido.' : email.length > 100 ? 'Correo permite máximo 100 caracteres.' : !TCG.allowedEmail(email) ? 'Usa @duoc.cl, @profesor.duoc.cl o @gmail.com.' : ''],
          [form.elements.region, !region ? 'Selecciona una región.' : ''],
          [form.elements.commune, !commune ? 'Selecciona una comuna.' : ''],
          [form.elements.address, !address ? 'Dirección es requerida.' : address.length > 300 ? 'Dirección permite máximo 300 caracteres.' : ''],
          [form.elements.password, password && (password.length < 4 || password.length > 10) ? 'La contraseña debe tener entre 4 y 10 caracteres.' : '']
        ];
        ...
        const index = users.findIndex(user =>
          String(user.email || '').toLowerCase() === oldEmail || String(user.run || '').toUpperCase() === oldRun
        );
        if (index < 0) {
        ...
        const duplicatedEmail = users.some((user, i) => i !== index && String(user.email || '').toLowerCase() === email);
        const duplicatedRun = users.some((user, i) => i !== index && String(user.run || '').toUpperCase() === run);
        if (duplicatedEmail) {
        ...
        // Mantiene asociados al cliente los pedidos ya existentes si cambió nombre o correo.
        const orders = TCG.getOrders();
        let ordersChanged = false;
        orders.forEach(order => {
          const matchesOldEmail = order.customerEmail && String(order.customerEmail).toLowerCase() === oldEmail;
          const matchesOldName = String(order.customer || '').trim().toLowerCase() === oldName.toLowerCase();
  ...
  function renderOrders(user) {
    ...
    if (!orders.length) {
      target.innerHTML = '<div class="content-card empty-state"><h3>Aún no tienes pedidos</h3><p>Cuando realices una compra, aparecerá aquí junto a su estado.</p><a class="btn btn-primary" href="productos.html">Ver productos</a></div>';
      return;
    }
        ...
        ${orders.map(order => `
          <article class="client-order-card">
            <div>
              ...
              <p>${order.items.length} producto${order.items.length === 1 ? '' : 's'} · ${TCG.currency(order.total)}</p>
            </div>
            <div class="client-order-actions">
              <span class="badge ${statusClass(order.status)}">${escapeHtml(order.status)}</span>
              <a class="btn btn-secondary btn-sm" href="pedido.html?id=${encodeURIComponent(order.id)}">Ver detalle</a>
            </div>
          </article>`).join('')}
      </div>`;
  }
  ...
  function renderOrderDetail() {
    ...
    const order = TCG.getOrders().find(item => item.id === id && belongsToUser(item, session));
    if (!order) {
      target.innerHTML = '<div class="content-card empty-state"><h2>Pedido no encontrado</h2><p>El pedido no existe o no pertenece a tu cuenta.</p><a class="btn btn-primary" href="cuenta.html">Volver a Mi cuenta</a></div>';
      return;
    }

    const statuses = ['Pendiente', 'Preparando', 'Despachado', 'En ruta', 'Entregado'];
    let currentIndex = statuses.findIndex(status => status.toLowerCase() === String(order.status || '').toLowerCase());
          ...
          <div class="order-timeline">
            ${statuses.map((status, index) => `
              <div class="timeline-step ${index < currentIndex ? 'completed' : ''} ${index === currentIndex ? 'current' : ''}">
                <span class="timeline-dot">${index < currentIndex ? '✓' : index + 1}</span>
                <span>${status}</span>
              </div>`).join('')}
          </div>
        </section>
          ...
          <div class="order-items-list">
            ${order.items.map(item => `
              <div class="order-item-line">
                <div><strong>${escapeHtml(item.name)}</strong><span>${item.qty} × ${TCG.currency(item.price)}</span></div>
                <strong>${TCG.currency(Number(item.qty) * Number(item.price))}</strong>
              </div>`).join('')}
          </div>
          <div class="summary-line total"><span>Total</span><span>${TCG.currency(order.total)}</span></div>
          <a class="btn btn-secondary account-back" href="cuenta.html">← Volver a Mi cuenta</a>
        </section>
      </div>`;
  }
